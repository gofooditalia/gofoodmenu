...
async function seed() {
    ...
    const { error: deleteDishesError } = await supabase.from('dishes').delete().eq('restaurant_id', restaurantId);
    if (deleteDishesError) console.error('Error deleting dishes:', deleteDishesError);

    const { error: deleteCatsError } = await supabase.from('categories').delete().eq('restaurant_id', restaurantId);
